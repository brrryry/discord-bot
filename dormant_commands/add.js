...
exports.run = async (client, message, args, level) => {
  ...
  let addCurrencyPromise = new Promise(resolve => {
    ...
    db.all(`SELECT * FROM currency WHERE id = ${message.mentions.members.first().id} AND guild = ${message.guild.id}`, (err, rows) => {
      if(rows.length == 0) {
        db.run(`INSERT INTO currency (id, guild, currency, prestige) VALUES (?, ?, ?, ?)`, [message.mentions.members.first().id, message.guild.id, currencyamount, 0]);
      } else {
        rows.forEach(row => {
  ...
  message.channel.send("Currency succesfully added!");
...
exports.config = {
  name: "add",
  usage: "add <user ping> <amount>",
  description: "Add currency to someone's account!",
  category: "currency",
  permissionLevel: 5
};
