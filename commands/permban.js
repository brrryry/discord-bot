exports.run = async (client, message, args, level) => {
  ...
  var now = new Date().toLocaleDateString("en-US", {
    ...
    db.run(`INSERT INTO modlogs (guild, moderator, offender, modtype, muteTime, reason, time) VALUES (?, ?, ?, ?, ?, ?, ?)`, [message.guild.id, message.author.id, user.id, "uban", 0, reason, now]);
    ...
    message.mentions.users.first().send("You were banned (unappealable) for: " + reason);
    message.mentions.member.first().ban();
